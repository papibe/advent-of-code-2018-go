...
func solve(instructions []Instruction, intruction_pointer, register0 int) int {
	...
	for {
		...
		// finding a cycle to runs as many times as possible
		if program_pointer == 28 {
			_, seen_in_reg4 := seen_reg4[registers[4]]
			if seen_in_reg4 {
				return prev_reg4
			} else {
				seen_reg4[registers[4]] = true
				prev_reg4 = registers[4]
			}
		}
...
func main() {
	fmt.Println(solution("input.txt")) // 10199686
}
