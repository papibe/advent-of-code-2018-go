...
func solve(instructions []Instruction, intruction_pointer, register0 int) int {
	...
	for {
		...
		program_pointer = registers[intruction_pointer]
		if registers[0] != last_value {
			last_value = registers[0]
		}
	}
	return registers[0]
}

func solution(filename string, register0 int) int {
	instructions, instruction_register := parse(filename)
	return solve(instructions, instruction_register, register0)
}

func main() {
	fmt.Println(solution("example.txt", 0)) // 7
	fmt.Println(solution("input.txt", 0))   // 1872
}
