...
func chose_target(unit *Unit, units []*Unit) (*Unit, int, bool) {
	...
	if len(damages) > 0 && damages[0].damage > 0 {
...
func solve(immunes, infections []Unit) int {
	all_units := []*Unit{}
	...
	return units_reminding
}

func solution(filename string) int {
	immunes, infections := parse(filename)
	return solve(immunes, infections)
}

func main() {
	fmt.Println(solution("./example.txt")) // 5216
	fmt.Println(solution("./input.txt"))   // 26343
}
