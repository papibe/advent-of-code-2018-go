...
func create_list(polymer string, exclude_char rune) *Node {
	dummy_head := &Node{unit: '>', next: nil, prev: nil}
	current := dummy_head

	for _, char := range polymer {
		if char == exclude_char || char == unicode.ToUpper(exclude_char) {
			continue
		}
...
func solution(filename string) int {
	...
	for letter := 'a'; letter <= 'z'; letter++ {
		polymer_list := create_list(polymer, letter)
		length := solve(polymer_list)
